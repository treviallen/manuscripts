...
for ii, ncf in enumerate(ncfiles):
    ...
    m.drawparallels(arange(-90.,90.,2), labels=[0,0,0,0],fontsize=10, dashes=[2, 2], color='0.5', linewidth=0.5)
    m.drawmeridians(arange(0.,360.,2), labels=[0,0,0,0], fontsize=10, dashes=[2, 2], color='0.5', linewidth=0.5)
    ...
    ade_lon = 138.6
    ade_lat = -34.93
    x, y = m(ade_lon, ade_lat)
    plt.plot(x, y, 's', markerfacecolor='none', markeredgecolor='k', markeredgewidth=1.5, \
         markersize=9)

    # label cities
    off = 0.25
    x, y = m(ade_lon-0.14, ade_lat+0.1)
    plt.text(x, y, 'Adelaide', size=16, horizontalalignment='right', weight='normal', path_effects=path_effects)
...
cax = figure.add_axes([0.2,0.05,0.6,0.05]) # setup colorbar axes.
