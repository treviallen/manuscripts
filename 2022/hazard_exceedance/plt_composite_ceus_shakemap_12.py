...
ncfiles = ['max_pga_grid_ceus_banda.grd']
           
pltlett = ['a)', 'b)', 'c)', 'd)', 'e)']
modnames = ['AU & CEUS GMMs']
...
# set colourbar
cax_bounds = ax.get_position().bounds
cbar_start = cax_bounds[0] + 0.1 * cax_bounds[2]
cbar_len = 0.8 * cax_bounds[2]
plt.gcf().subplots_adjust(bottom=0.105)
cax = figure.add_axes([cbar_start,0.14,cbar_len,0.03]) # setup colorbar axes.

cb = colorbar.ColorbarBase(cax, cmap=cmap, norm=norm, orientation='horizontal')

cb.set_ticks(bounds[::2])
labels = ['0'+str('%0.3f' % x).strip('0') for x in bounds[::2]]
labels[0] = '0.0'
...
##############################################################################
# make difference grid and show
##############################################################################

system('gmt5 grdmath max_pga_grid_ceus_banda.grd max_pga_grid_au_banda.grd SUB ABS au_land_mask.grd MUL = sm_diff_haz.grd') #au_land_mask.grd MUL 
...
# get colormap from cpt file
cptfile = '/Users/trev/Documents/DATA/GMT/cpt/tempo.cpt'
ncolours = 10
...
# now save png file
plt.savefig(path.join('figures/composite_ceus_shakemap_diff.png'), dpi=300, format='png', bbox_inches='tight')
