...
###############################################################################
doADE = True # check scenario in data folder!!!!
datafolder = 'shakemap/aus_gmm_output'
#datafolder = 'shakemap/aus_ceus_gmm_output'

#datasets = ['shakemap/banda_output', datafolder] # only run once - banda already made
datasets = [datafolder]
# loop thru folders
for ii, dataset in enumerate(datasets):

    folders = get_folder_list(path.join(dataset, 'data'))
    smtxt = 'DATETIME,LON,LAT,DEP,MW\n'
        
    for folder in folders:
        print(folder)
        # set xml path
        xmlfile = path.join(dataset, 'data', folder, 'current', 'products', 'grid.xml')
        print(xmlfile)
        ...
        try:
            ...
            # write to txt file
            if dataset.endswith('aus_gmm_output') or dataset.endswith('banda_output'):
                sm_lons = griddata[:,0]
                sm_lats = griddata[:,1]
                sm_pgas = griddata[:,3]
            else:
    ...
    csvfile = 'shakemap_event_list_'+suffix+'.csv'
    f = open(csvfile, 'w')
    f.write(smtxt)
    f.close()
    ###############################################################################
    ...
    if dataset.endswith('aus_gmm_output'):        
        if doADE == False:
            f = open('max_pga_grd_au.txt', 'w')
        else:
            f = open('max_pga_grd_ade.txt', 'w')
    elif dataset.endswith('banda_output'):        
        f = open('max_pga_grd_banda.txt', 'w')
    else:
    ...
    if dataset.endswith('aus_gmm_output'):
        if doADE == False:
            ...
            system('gmt5 grdmath max_pga_grid_au.grd max_pga_grid_banda.grd MAX = max_pga_grid_au_banda.grd')
        else:
            ...
            system('gmt5 grdmath max_pga_grid_ade.grd max_pga_grid_banda.grd MAX = max_pga_grid_ade_banda.grd')
        
    elif dataset.endswith('banda_output'):
        system('gmt5 surface max_pga_grd_banda.txt -Gmax_pga_grid_banda.grd -R110/156/-46/-9 -I'+str(res))
        system('gmt5 grdmath max_pga_grid_banda.grd 0 MAX = max_pga_grid_banda.grd')
    else:
        ...
        system('gmt5 grdmath max_pga_grid_ceus.grd max_pga_grid_banda.grd MAX = max_pga_grid_ceus_banda.grd') # max of ceus & banda grds
