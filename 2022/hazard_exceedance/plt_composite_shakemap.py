...
maprows = 1
mapcols = 1
figure = plt.figure(1,figsize=(7.5,8))
...
ncfiles = ['max_pga_grid_au_banda.grd']
...
for ii, ncf in enumerate(ncfiles):
    ...
    ylim = ax.get_ylim()
    ytxt = ylim[1] * 0.02
    plt.text(xtxt, ytxt, modnames[ii], fontsize=18, va='bottom', ha='left')
...
cax = figure.add_axes([0.15,0.125,0.7,0.032]) # setup colorbar axes.
...
'''    
# now save png file
plt.savefig(path.join('figures/composite_au_shakemap.png'), dpi=300, format='png', bbox_inches='tight')
