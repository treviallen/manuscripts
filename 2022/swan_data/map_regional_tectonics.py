...
m = Basemap(projection='lcc',lat_1=lat_1,lat_2=lat_2,lon_0=lon_0,\
            ...
            rsphere=6371200.,resolution='f',area_thresh=500.) #i
...
m.drawparallels(arange(-90.,90.,2.), labels=[1,0,0,0],fontsize=13, dashes=[2, 2], color='0.5', linewidth=0.75)
m.drawmeridians(arange(0.,360.,3.), labels=[0,0,0,1], fontsize=13, dashes=[2, 2], color='0.5', linewidth=0.75)
...
shpfile = '/Users/trev/Documents/DATA/GIS/au_tectonic_provinces/Tectonic_provinces_PB_AFO_YIL.shp'
sf = shapefile.Reader(shpfile)

shapes = sf.shapes()
provs = get_field_data(sf, 'NAME', 'str')
#utrts = unique(trts)

for c, prov, shape in zip(cs, provs, shapes):
    labeLegend = True
    x = []
    y = []
    ...
    for prt in parts[1:]:
        ...
        #print(edgecolor)
        if labeLegend == True:
            plt.fill(xx,yy, facecolor=c, edgecolor='none', linewidth=0.75, alpha=0.3, label=prov)
            labeLegend = False
        else:
...
plt.savefig('swan_tectonic_setting.png',fmt='png',dpi=300,bbox_inches='tight')
