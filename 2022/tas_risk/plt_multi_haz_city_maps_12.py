...
for ii, hazfile in enumerate(hazfiles):
    ...
    for i, key in enumerate([keys[mapidx]]): # just plot 1 for now!
        ...
        m.drawparallels(arange(-90.,90.,0.1), labels=[0,0,0,0],fontsize=10, dashes=[2, 2], color='0.5', linewidth=0.5)
        m.drawmeridians(arange(0.,360.,0.1), labels=[0,0,0,0], fontsize=10, dashes=[2, 2], color='0.5', linewidth=0.5)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    ncicptfile = '/Users/trev/Documents/Geoscience_Australia/NSHA2018/postprocessing/maps/'+ cptfile
                    cmap, zvals = cpt2colormap(ncicptfile, ncolours, rev=True)
        ...
        if probability == '10%' or probability == '9.5%':
            if pltGSHAP == 'True':
                ...
                bounds = array([0.005, 0.006, 0.007, 0.008, 0.009, 0.01, 0.011, 0.012, 0.013, 0.014, 0.015, 0.02, 0.04])
                ncolours = 12
        ...
        ylim = ax.get_ylim()
        ytxt = ylim[1] * 0.96
        plt.text(xtxt, ytxt, pltlett[ii], fontsize=26, va='top', ha='left')
        
        xtxt = xlim[1] * 0.02
        ylim = ax.get_ylim()
        ytxt = ylim[1] * 0.02
        plt.text(xtxt, ytxt, modnames[ii], fontsize=20, va='bottom', ha='left')
        
        ##########################################################################################
        # plt subduction profile - comment out when not using
        ##########################################################################################
        '''
        proffile = '/Users/trev/Documents/Geoscience_Australia/NSHA2018/shared/north_aus_profile.csv'
        proflons = []
        proflats = []
