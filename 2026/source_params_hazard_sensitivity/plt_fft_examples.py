...
def parse_pickfile(pickfile):
    ...
    # parse pick file
    folder = '../../2023/au_stress_drop/record_picks'
...
evdict = parse_ga_event_query('../../2023/au_stress_drop/au_ge_4.4_earthquakes_export_edit.csv')
...
pickfiles = ['2018-09-16T04.52.IU.NWAO.picks', '2024-08-23T01.59.AU.NTLH.picks']
...
for p, pf in enumerate(pickfiles[start_idx:]):
    ...
    if isnan(pickDat['mag']) == False:
        ...
        try:
            ...
            mseedpath = pickDat['mseed_path'].replace('mseed_dump','iris_dump')
            st = read(mseedpath)
            st = st.merge()
        except:
            try:
                ...
                st = read(path.join('/Users/trev/Documents/Manuscripts/manuscripts/2023/au_stress_drop/iris_dump/', mseedfile))
            except:
        ...
        if skipRec == False:
            ...
            for tr in new_st:
                ...
                if tr.stats.channel.endswith('Z'):
                    ...
                    plt.loglog(freqs, n_disp_amp, '-', c=cols[0], lw=0.5)
                    plt.loglog(interp_freqs, smoothed_interp_disp, '-', c=cols[1], lw=1, label='Noise', zorder=10)
                    ...
                    plt.loglog(freqs, ps_disp_amp, '-', c=cols[2], lw=0.5)
                    plt.loglog(interp_freqs, smoothed_interp_disp, '-', c=cols[3], label='P & S-Phase', zorder=11)
                    
                    '''
                    # get s-wave fft of trace
                    '''
                    
                    sttr = tr.stats.starttime + pickDat['sidx'] * tr.stats.delta - 1. # allow buffer
                    ...
                    plt.loglog(freqs, s_disp_amp, '-', c=cols[4], lw=0.5)#plt.xlim([0.05, 10])
                    plt.loglog(interp_freqs, smoothed_interp_disp, '-', c=cols[5], lw=1, label='S-Phase', zorder=12)
                    
                    '''
                    # get s- and rg-wave fft of trace
                    '''
                    
                    sttr = tr.stats.starttime + pickDat['sidx'] * tr.stats.delta - 1. # allow buffer
                    ettr = tr.stats.starttime + pickDat['eidx'] * tr.stats.delta + 20. # allow buffer
                    ...
                    # get instrument corrected spectra
                    try:
                        freqs, rg_disp_amp, pazfile = response_corrected_fft(str_trim, pickDat)
                    except:
                        freqs, rg_disp_amp, pazfile = retry_stationlist_fft(str_trim, pickDat)
                        
                    smoothed_disp, smoothed_interp_disp = get_smoothed_fft_spectra(freqs, rg_disp_amp)
                    
                    traceDat['rgwave_spec'] = smoothed_interp_disp
                    
                    plt.loglog(freqs, rg_disp_amp, '-', c=cols[6], lw=0.5)#plt.xlim([0.05, 10])
                    plt.loglog(interp_freqs, smoothed_interp_disp, '-', c=cols[7], lw=1, label='S & Rg-Phase', zorder=13)
                    
                    if p == 0:
                        plt.legend(loc=3, fontsize=11)
                        plt.ylabel('Fourier Displacement (m-s)', fontsize=12)
                    plt.xlabel('Frequency (Hz)', fontsize=12)
                    plt.grid(which='major')
                    plt.xticks(fontsize=10)
                    plt.yticks(fontsize=10)
                    title = str(UTCDateTime(pickDat['origintime']))[0:19] +' '+tr.stats.station
                    plt.title(title, fontsize=10)
                    plt.gcf().set_size_inches(11, 5)
                    plt.xlim([0.01, 60])
            
plt.savefig('figures/exmple_rg_fft.png', bbox_inches='tight', dpi=300)
plt.show()        
