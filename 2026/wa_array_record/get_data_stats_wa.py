...
for i, net in enumerate(nets_dict):
    prec_cnt = 100*net['cnt']/nrecs
    print(i, net['net'], net['cnt'], str('%0.1f' % prec_cnt)+'%')
...
redlabs = set(['AU', 'S1', '2P', 'WG'])
barcolors = ["r" if i in redlabs else "0.7" for i in net_lab]
ax.bar(net_lab, net_cnt, color=barcolors) #fc='0.7')
ax.set_yscale('log')
