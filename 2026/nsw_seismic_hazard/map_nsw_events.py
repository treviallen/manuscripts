...
m = Basemap(llcrnrlon=llcrnrlon,llcrnrlat=llcrnrlat,urcrnrlon=urcrnrlon,urcrnrlat=urcrnrlat, \
            resolution='h', epsg=3857, area_thresh=200.)

m.drawparallels(arange(-90.,90.,2), labels=[1,0,0,0],fontsize=12, dashes=[2, 2], color='0.5', linewidth=0.0)
m.drawmeridians(arange(0.,360.,2), labels=[0,0,0,1], fontsize=12, dashes=[2, 2], color='0.5', linewidth=0.0)
...
for line in lines:
    ...
    evdict.append(tmp) 
'''
evdict = parse_ga_event_query('nsw_earthquakes_export.csv')
...
for lm in legmag:
    x, y = m(0, 0)
    h = m.plot(x, y, 'o', mfc='k', mec='k', mew=0.25, markersize=(2.75 * lm - 1), alpha=1., zorder=len(mags)+1)
    legh.append(h[0])

legtxt = ('$\mathregular{M}$ 2.0', '$\mathregular{M}$ 3.5', '$\mathregular{M}$ 5.0')
l = plt.legend(legh, legtxt, loc=2, numpoints=1, fontsize=12, title="Magnitude", title_fontsize=16, \
               labelspacing=0.75, frameon=True)
...
cax = fig.add_axes([0.25,0.045,0.5,0.025]) # setup colorbar axes.
...
cb.set_label('Year of Earthquake', rotation=0, fontsize=16, labelpad=5, color='k')
...
axins = zoomed_inset_axes(ax1, 0.05, loc=4)
...
# fill main area
xv = [llcrnrlon, llcrnrlon, urcrnrlon, urcrnrlon, llcrnrlon]
yv = [llcrnrlat, urcrnrlat, urcrnrlat, llcrnrlat, llcrnrlat]
x, y = m2(xv, yv)
...
plt.savefig('nsw_eq_map_satellite.png',format='png',dpi=300,bbox_inches='tight')
