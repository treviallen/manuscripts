...
from numpy import array, sqrt, nan, isnan, arange, abs, unique, hstack, savetxt, corrcoef, vstack, \
                  logical_and, mean, median, std, log10, ones, logspace, exp, max, signbit, log, exp, linspace, interp
...
for i in range(0, len(fmag)):
    sx = interp(fmag[i], im, isi)
    sy = sx

    data = odrpack.RealData(array(fmag[i]),log10(array(flen[i])), sx=sx, sy=sy)
    grad = grads[0]
    
    if i == 0:
        # fit slope for in-slab
        #lin = odrpack.Model(linreg)
        beta0=[-2., 1.]
        #odr = odrpack.ODR(data, lin, beta0=[-2., 1.])
        odr = ODR(data,linreg,beta0)
    ...
    c = out.beta
    if i == 0:
        savetxt('intra_len.txt',c,delimiter=' ')
    else:
        savec.append(c[0])
    regmrng = arange(min(fmag[i]), max(fmag[i])+0.01, 0.01)
    if  i == 0:
        clen = 10**(c[0] + c[1] * regmrng)
    else:
    ...
    tabtxt += ','.join((ftypes[i], str('%0.2f' % c[0]), str('%0.2f' % out.sd_beta[0]), \
...
for i in range(0, len(fmag)):
    ...
    data = odrpack.RealData(array(fmag[i]),log10(array(fwid[i])), sx=sx, sy=sy)
    grad = grads[1]
    ...
    c = out.beta
    if i == 0:
        savetxt('intra_wid.txt',c,delimiter=' ')
    else:
        savec.append(c[0])
    regmrng = arange(min(fmag[i]), max(fmag[i])+0.01, 0.01)
    if  i == 0:
        cwid = 10**(c[0] + c[1] * regmrng)
    else:
        cwid = 10**(c + grad * regmrng)
    print i
    h = plt.semilogy(regmrng, cwid, '-', color=cs2[i+1], lw=2.0)
    hh.append(h[0])
