...
evdict = parse_ga_event_query('1960_swwa_earthquakes_export.csv')
...
# annotate cities
numCities = 15
blacklist = ['Ellenbrook']
annotate_cities(numCities, plt, m, markerfacecolor='none', markeredgecolor='r', \
                marker='s', markersize=6, markeredgewidth=1.5, fs=11, weight='normal', blacklist=blacklist)

# Add ramsey locs
blacklist = ['Northam', 'Merredin', 'Forrestfield', 'Bindoon', 'Cunderdin', 'Southern Cross']
filterloc=[116, 121, -32., -30.5]
annotate_cities(4, plt, m, markerfacecolor='none', markeredgecolor='r', \
                marker='s', markersize=6, markeredgewidth=1.5, fs=11, \
                blacklist=blacklist, filterloc=filterloc, loc=4)

x, y = m(120, -35)
plt.text(x, y, 'SOUTHERN\nOCEAN', size=13, c='dodgerblue', va='center', ha='center', weight='light', \
...
axins = zoomed_inset_axes(ax, 0.028, loc=1)
