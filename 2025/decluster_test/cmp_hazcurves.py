...
if __name__ == "__main__":
    ...
    job_num        = split_config_lines(lines[7])
    ...
    for p, pp in enumerate(plt_places):
        ...
        for j, relativepath in enumerate(relativepaths):
            ...
            '''
            hazcurvefile = path.join(relativepath, 'hazard_curve-mean-PGA_'+job_num[j]+'.csv')
            siteDict, imls, investigation_time = return_annualised_haz_curves(hazcurvefile)
            ...
            if relativepath.startswith('/Users/'):
                fractfile16 = path.join(relativepath, 'quantile_curve-0.16-PGA_'+job_num[j]+'.csv')
                siteDict16, imls, investigation_time = return_annualised_haz_curves(fractfile16)
                
                fractfile84 = path.join(relativepath, 'quantile_curve-0.84-PGA_'+job_num[j]+'.csv')
                        ...
                        if around(plon, decimals=2) == around(sd16['lon'], decimals=2) \
                           and around(plat, decimals=2) == around(sd16['lat'], decimals=2):
                            ...
                            if pp == place:
                                ...
                                h = plt.loglog(imls, sd16['poe_probs_annual'], '--', color=cols[j], lw=1.5, label='16-84th Percentile')
