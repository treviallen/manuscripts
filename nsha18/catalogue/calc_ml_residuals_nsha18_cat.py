...
hvrat = []
hmax = []
lines = open('seaust_w-a_amps_post2009.csv').readlines()
...
def plt_stats(plt, rhyp, res):
    ...
    strng = 'Med = ' + str('%0.2f' % meanres) + '\n' \
          + 'Std  = ' + str('%0.2f' % stdres)
    plt.text(660, -0.61, strng, fontsize=12, va='bottom')

mmin = [2.0, 2.0, 3.0, 4.0]
mmax = [6.0, 3.0, 4.0, 6.0]
...
for mi, ma in zip(mmin, mmax):
    ...
    # save fig
    pngfile = 'intra_eqn_res_nshacat.'+str(mi)[0]+'-'+str(ma)[0]+'.png'
