...
# parse epicentres
##########################################################################################
ml_diff_file = 'ML_diff.dat'

# parse HMTK csv
lines = open(ml_diff_file).readlines()
evla = []
evlo = []
mldf = []
mlrv = []
...
for line in lines:
    ...
    evlo.append(float(dat[0]))
    evla.append(float(dat[1]))
    mldf.append(float(dat[2]))
    mlrv.append(float(dat[3])*15.) # /15 in mlab code
...
l = plt.legend(legh, ('ML 3.0', 'ML 5.0', 'ML 7.0'), loc=1, numpoints=1)
...
plt.savefig('2018_aus_ml_diff.png', format='png', bbox_inches='tight', dpi=200)
plt.show()
