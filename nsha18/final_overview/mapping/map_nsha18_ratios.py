...
# set lims
clim = float(argv[4]) # valid inputs 3 or 4
...
for i, key in enumerate([keys[0]]): # just plot 1 for now!
    ...
    # now save png file
    print 'Saving', path.join('map_ratio', 'map_ratio_'+modelName.replace(' ','_')+'.'+key+'.png')
    plt.savefig(path.join('map_ratio', 'map_ratio_'+modelName.replace(' ','_')+'.'+key+'.png'), \
