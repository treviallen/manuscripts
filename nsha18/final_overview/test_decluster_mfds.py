...
for line in lines:
    ...
    try:
        tmpDict = {'datetime':datetime.strptime(dat[0], '%Y%m%d%H%M'), 'prefmag':float(dat[-1]), \
                   'lon':float(dat[7]), 'lat':float(dat[8]), 'mx_origML':float(dat[-1])}
    except:
        tmpDict = {'datetime':datetime.strptime(dat[0], '%Y%m%d%H%M'), 'prefmag':nan, \
                   'lon':float(dat[7]), 'lat':float(dat[8]), 'mx_origML':nan}
    
    fullCat.append(tmpDict)
        ...
        for j, ck in enumerate(catKeys):
            ...
            bval, beta, sigb, sigbeta, fn0, cum_rates, ev_out, err_up, err_lo = \
                  get_mfds(mvect, mxvect, tvect, dec_tvect, ev_dict, \
                           mcomps, ycomps, nshaMaxYear, mrng, src_mmax[i], 3.05, \
