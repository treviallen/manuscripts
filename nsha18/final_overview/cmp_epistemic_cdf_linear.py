    ...
    for i, place in enumerate(places):
        ...
        for frac1, frac2 in zip(fracDict1, fracDict2):
            if place == frac1['place']:
                ...
                plt.plot(frac1['quant_'+key], fractiles, 'k-', lw=1.5, label=modnames[0])
                plt.plot(frac2['quant_'+key], fractiles, 'r-', lw=1.5, label=modnames[1])
                ...
                # plt mean
                plt.plot([frac1['mean_'+key],frac1['mean_'+key]], [0,1], '--', c='seagreen', lw=1.5, label='Mean')
                plt.plot([frac2['mean_'+key],frac2['mean_'+key]], [0,1], '--', c='orangered', lw=1.5, label='Mean')
                '''
                # plt median
                plt.plot([frac['quant_'+key][50],frac['quant_'+key][50]], [0,1], '--', c='dodgerblue', lw=1.5, label='50th Percentile')
                # plt 84th
                plt.plot([frac['quant_'+key][84],frac['quant_'+key][84]], [0,1], '--', c='orange', lw=1.5, label='84th Percentile')
                # plt 95th
                plt.plot([frac['quant_'+key][95],frac['quant_'+key][95]], [0,1], '--', c='r', lw=1.5, label='95th Percentile')
                '''
