def get_pkl_geomean(stndat):
    ...
    try:
        ...
        geomean = sqrt(log(stndat['epsa']) * log(stndat['npsa'])) # in g
    except:
...
from numpy import array, interp, sqrt, exp, log, nan, zeros_like, where
from calc_oq_gmpes import crustal_gsims
from gmpe_statistics import get_loglikelihood_values
import warnings
...
for root, dirnames, filenames in walk('pkl_files'):
   for filename in filter(filenames,'*_2.pkl'):   
        ...
        for i, stndat in enumerate(psadat):
            ...
            if mag >= 5.0 and rrup <= 100  and stndat['stn']!= 'LIB':
                ...
                if time == True:
                    if eqtime <= hg_time:
                        Bea97_stack = stack_residuals(Bea97_stack, Bea97imt, stndat, geomean)
                        Zea06_stack = stack_residuals(Zea06_stack, Zea06imt, stndat, geomean)
                        CB08_stack = stack_residuals(CB08_stack, CB08imt, stndat, geomean)
                        CY08_stack = stack_residuals(CY08_stack, CY08imt, stndat, geomean)
                        Bea11_stack = stack_residuals(Bea11_stack, Bea11imt, stndat, geomean)
                        BA11_stack = stack_residuals(BA11_stack, BA11imt, stndat, geomean)
                        AA13_stack = stack_residuals(AA13_stack, AA13imt, stndat, geomean)
                        Aea14_stack = stack_residuals(Aea14_stack, Aea14imt, stndat, geomean)
                        Bea14_stack = stack_residuals(Bea14_stack, Bea14imt, stndat, geomean)
                        CY14_stack = stack_residuals(CY14_stack, CY14imt, stndat, geomean) 
...
for j, model in enumerate(models):
    ...
    llh, tllh, per = get_loglikelihood_values(eval(model+'_stack'), array(periods))
    
    print models_ext[j], llh
    print tllh
   
    moddict = {'llh':llh, 'tllh':tllh, 'per':per}
    llhdict[model] = moddict
    
    # plot LLH with period
    plt.semilogx(per, tllh)
    ...
    '''
if time == True:
    plt.savefig('LLH_ranking.pre.pdf', format='pdf', dpi=150)
else:
    plt.savefig('LLH_ranking.all.pdf', format='pdf', dpi=150)
plt.show()
