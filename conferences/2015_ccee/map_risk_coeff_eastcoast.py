...
drawshapepoly(m, plt, sf, col='lightgray', fillshape=True, lw=0.15)
'''
# read risk ceof
lat = []
lon = []
...
keylocs = ['Ottawa', 'London', 'Toronto', 'Montreal', 'Hamilton', 'Windsor', 'Sudbury', 'Timmins', \
           'Kingston', 'Kitchener', 'Kingston', 'Saint John', 'Dartmouth', \
           "St. John's", 'Brampton', 'Quebec City', 'Laval', 'Gatineau', \
           'Sherbrooke', 'Saguenay', 'Fredericton', 'Moncton']

x, y = m(array(lon), array(lat))
m.scatter(x, y, c=riskcoeff, s=80, cmap=plt.cm.Spectral_r, vmin=0.84, vmax=1.0, alpha=1.)
...
cax = figure.add_axes([0.91,0.25,0.015,0.5])
...
plt.savefig(outpdf+'_eastcoast.pdf' ,format='pdf', dpi=300)
