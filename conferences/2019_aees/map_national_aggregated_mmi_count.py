...
# loop trhu min-max years
    ...
    while lon_grd < maxlon:
        ...
        while lat_grd < maxlat:
           ...
           if len(idx) > 0:
               ...
               # get uniue events
               ids = event_ids[idx]
               grd_count.append(len(unique(ids)))
    ...
    bounds = array([1, 3, 5, 10, 15, 20, 200])
    ...
    # set cb labels
    ticks = arange(0.5,6)
    cnt_rng = ['1', '2-3', '4-5', '6-10', '11-15', '16+']
    cb.set_ticks(ticks)
    ...
    plt.savefig(str(year)+'_map_nat_agg_mmi_count.png', fmt='png', dpi=300, bbox_inches='tight')
plt.show()