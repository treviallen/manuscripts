...
shpfile = '2025_contours/AS1170_4_fitted_modified_contours.shp'
...
y = [0, 1, 1, 0, 0]
xx, yy = m(x,y)
plt.fill(xx,yy, fc='w',edgecolor='k', linewidth=0.75, alpha=0.3, label='0.08')

plt_levs = [0.08, 0.1]

for pl in plt_levs:
    labeLegend = True
    for shape, levs in zip(shapes, levels):
        if levs == pl:
            x = []
            y = []
            ...
            for prt in parts[1:]:
                ...
                if labeLegend == True:
                    if pl == 0.08:
                        plt.fill(xx,yy, fc='w', hatch='///', edgecolor='0.3', linewidth=0.75, alpha=0.3, \
                                 label='0.10')
                    else:
                        plt.fill(xx,yy, fc='w', edgecolor='none', linewidth=0.75)
                        plt.fill(xx,yy, fc='w', hatch='...', edgecolor='0.3', linewidth=0.75, alpha=0.3, \
                                 label=str(round(pl+0.02, decimals=2)).zfill(2))
                    labeLegend = False
                else:
                    if pl == 0.08:
                        plt.fill(xx,yy, fc='w', hatch='///', edgecolor='k', linewidth=0.75, alpha=0.3)
                    else:
                        plt.fill(xx,yy, fc='w', edgecolor='none', linewidth=0.75)
                        plt.fill(xx,yy, fc='w', hatch='...', edgecolor='k', linewidth=0.75, alpha=0.3)
...
if not region == 'nat':
    ...
    if region == 'swwa':
        blacklist = ['Northam', 'Wongan Hills', 'Merredin', 'Kellerberrin', 'Cunderdin', 'York', 'Mukinbudin', 'Castletown']
        filterloc=[116, 119, -31.5, -30]
        annotate_cities(7, plt, m, markersize=5, marker='o', markeredgewidth=0., markerfacecolor='k', fs=12.5, \
                        blacklist=blacklist, filterloc=filterloc)
...
leg1 = plt.legend(loc=legloc, fontsize=16, title="Hazard-Design\nFactor (Z)")
...
plt.savefig(region+'_as1170_4_map.png', dpi=300, format='png', bbox_inches='tight')
