...
from hazard_tools import return_AS1170_4_shape, return_AS1170_4_2025_shape
...
per1170 = arange(0, 4.1, 0.05)
        
shp1170_b = return_AS1170_4_shape(per1170, 'B')
shp1170_b_2025 = return_AS1170_4_2025_shape(per1170, 'B')
'''
...
rat05 = norm_meanUHS_b[6] / shp1170_b_2025[10]
print(rat05)

plt.plot([0.5, 0.5], [norm_meanUHS_b[6], shp1170_b_2025[10]], 'k-', lw=3, zorder=0)
plt.plot([0.47, 0.53], [norm_meanUHS_b[6], norm_meanUHS_b[6]], 'k-', lw=3, zorder=0)
plt.plot([0.47, 0.53], [shp1170_b_2025[10], shp1170_b_2025[10]], 'k-', lw=3, zorder=0)

plt.xlabel('Period (s)', fontsize=14)
plt.ylabel('Normalised '+str('%0.1f' % (100*float(probabilities[probkey3pc])))+'% in 50-year AEP Sa', fontsize=14)
plt.grid(which='both')
plt.xlim([0.0, 3.0])
