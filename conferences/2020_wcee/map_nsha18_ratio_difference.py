...
nsha_b_file = 'hazard_grids/hazard_map-mean_1.csv'
grid_b_file = 'hazard_grids/hazard_map-mean_1_gridded.csv'

# get map name for plotting
modelName = 'NSHA18_ratios'

# set lims
clim = 2. # valid inputs 3 or 4
...
grddict1, keys  = parse_grd_file(grid_b_file)
grddict2, keys  = parse_grd_file(nsha_b_file)
    ...
    if gd1['lon'] == gd2['lon'] and gd1['lat'] == gd2['lat']:
        if gd2[keys[0]] == 0.0:
            tdict = {'lon': gd1['lon'], 'lat': gd1['lat'],
                     'logratio': nan, 'absdiff': nan}
        else:
             tdict = {'lon': gd1['lon'], 'lat': gd1['lat'],
                     'logratio': log10(gd1[keys[0]] / gd2[keys[0]]),
                     'absdiff': gd1[keys[0]] - gd2[keys[0]]}
...
for i, key in enumerate([keys[0]]): # just plot 1 for now!
    ...
    ylim = ax.get_ylim()
    ytxt = ylim[1] * 0.02
    plt.text(xtxt, ytxt, pltlett[0], fontsize=24, va='bottom', ha='left')   
...
cbar_start = bounds[0] + 0.15 * bounds[2]
cbar_len = 0.7 * bounds[2]
plt.gcf().subplots_adjust(bottom=0.1)
cax = fig.add_axes([cbar_start,0.14,cbar_len,0.025]) # setup colorbar axes.
...
# set title
titlestr = 'PGA Hazard Ratio'
cb.set_label(titlestr, fontsize=20)

##############################################################################    
# now make difference map
##############################################################################
...
for i, key in enumerate([keys[0]]): # just plot 1 for now!
    ...
    for gridval in grddict:
        lonlist.append(gridval['lon'])
        latlist.append(gridval['lat'])
        if isnan(gridval['absdiff']):
            logRatio.append(nan)
        else:
            logRatio.append(gridval['absdiff'])
    ...
    # get colormap from cpt file
    cptfile = 'sunshine_diff_12lev.cpt'
    cptfile = 'PRGn_11.cpt'
    
    clim =  0.015 # valid inputs 3 or 4
    if clim == 0.015:
        ncolours = 14
        nticks = 7
    elif clim == 3.:
    ...
    ylim = ax.get_ylim()
    ytxt = ylim[1] * 0.02
    plt.text(xtxt, ytxt, pltlett[1], fontsize=24, va='bottom', ha='left')
...
# set cb labels
#nticks = ceil(ncolours/2.)
linticks = linspace(vmin, vmax, nticks)
#logticks = log10(linticks)
cb.set_ticks(linticks)
labels = [str('%0.3f' % x) for x in linticks]
cb.set_ticklabels(labels)
cb.ax.tick_params(labelsize=16)

# set title
titlestr = 'PGA Hazard Difference'
cb.set_label(titlestr, fontsize=20)
