...
m.drawmeridians(arange(0.,360.,2.), labels=[0,0,0,1], fontsize=13, dashes=[2, 2], color='0.5', linewidth=0.75)
...
evdict = parse_ga_event_query('sa_earthquakes_export.csv')
...
for i in range(0, len(mags)): #[0:100])):
    ...
    plt.plot(x, y, 'o', mfc=list(cs[colidx][0:3]), markeredgecolor='k', markeredgewidth=0.25, \
             markersize=(3.6 * mags[i] - 4), zorder=zo, alpha=0.8)
...
for lm in legmag:
    x, y = m(0, 0)
    h = m.plot(x, y, 'o', mfc='k', mec='w', mew=0.25, markersize=(3.6 * lm - 4), alpha=1., zorder=len(mags)+1)
    legh.append(h[0])

legtxt = ('$\mathregular{M}$ 2.0', '$\mathregular{M}$ 4.0', '$\mathregular{M}$ 6.0')
...
plt.savefig('sa_eq_map.png',fmt='png',dpi=300,bbox_inches='tight')
