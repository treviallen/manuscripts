function [ RTGM, RiskCoefficient ] = RTGM_Calculator_Ver131017( HazardCurve, Iplot )

%
% Author:  Nicolas Luco (nluco@usgs.gov)
% Last Revised:  17 October 2013
% References:
...
% Iplot            = Indicator for plotting RTGM iterations (0=No, 1=Yes)
%
% OUTPUT:
% =======
...
[ UpsampledHC ] = Upsample_Hazard_Curve( HazardCurve, UHGM );
...
%Q%disp( strvcat( ' ' , 'Iteration Summary', '-----------------' ) )
...
if Iplot
    Plot_RTGM_Calculation_Iterations_Ver120222( UpsampledHC, AFE4UHGM, UHGM, RTGMi, ...
...
function [ upsampledHC ] = Upsample_Hazard_Curve( originalHC, UHGM )

UPSAMPLING_FACTOR = 1.05;
SMALLEST_SA = min( min(originalHC.SAs), UHGM/10 );
LARGEST_SA = max( max(originalHC.SAs), UHGM*10 );
...
%disp(FragilityCurve) % TA addition
