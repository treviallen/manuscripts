...
norm = colors.BoundaryNorm(boundaries=arange(b_min, b_max+0.05, 0.1), ncolors=ncolours)
...
###############################################################################
resolutions = ['2', '3', '4', '5', '6', '8']
letters = ['a)', 'b)', 'c)', 'd)', 'e)', 'f)']

#loop thru resolutions
for i, res in enumerate(resolutions):

    shpfile  = '//Users//trev//Documents//Geoscience_Australia//NSHA2023//source_models//zones//2026_hex/Hex_' \
                +res+'D//shapefiles//Hex_'+res+'D_NSHA23.shp'
    ...
    # loop thru b values
    print(src_bval)
    for b, n0, code in zip(src_bval, src_n0, src_code):
        if not isnan(b):
    ...
    drawshapepoly(m2, plt, sf, cindex=cindex, cmap=cmap, ncolours=ncolours, fillshape=True, edgecolor='none', lw=1.)
    ...
    ylim = ax.get_ylim()
    ytxt = ylim[1] * 0.02
    lettxt = ' '.join((letters[i], res, 'degrees'))
    plt.text(xtxt, ytxt, lettxt, fontsize=30, va='bottom', ha='left')
...
cax = fig.add_axes([0.25,0.035,0.5,0.0175]) # setup colorbar axes.
...
ticks = arange(b_min, b_max+0.05, 0.1)
cb.set_ticks(ticks)
labels = [str('%0.1f' % x) for x in ticks]

#cb.set_ticklabels(labels, fontsize=10)
cb.ax.set_xticklabels(labels, fontsize=17)
cb.set_label('b-value', fontsize=20)
  
# save figure
plt.savefig('multi_hex_b_val_maps.png', format='png', dpi=300, bbox_inches='tight')
