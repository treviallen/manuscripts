...
clat = [49.120, 48.430, 49.250, 49.170, 49.700, 50.120, 50.670, 53.25, 53.26, 54.02, 54.07, 54.32, 53.92]
clon = [-125.880, -123.370, -123.120, -123.930, -123.150, -122.960, -120.320, -131.82, -132.08, -132.1, -128.65, -130.32, -122.75]
cloc = ['Tofino', 'Victoria', 'Vancouver', 'Nanaimo', 'Squamish', 'Whistler', 'Kamloops', 'Sandspit', 'Queen Charlotte', \
        'Masset', 'Kitimat', 'Prince Rupert', 'Prince George']
...
for i, loc in enumerate(cloc):
    if i == 1 or i == 7 or i == 2:
        x, y = m(clon[i]+0.12, clat[i]+0.1)
        txt = plt.text(x, y, loc, size=15, horizontalalignment='left', weight='normal')
    elif i == 12:
        x, y = m(clon[i]-0.09, clat[i]-0.1)
        txt = plt.text(x, y, loc, size=15, ha='right', va='top', weight='normal')
    else:
        x, y = m(clon[i]-0.12, clat[i]+0.1)
        txt = plt.text(x, y, loc, size=15, horizontalalignment='right', weight='normal')
...
axins = zoomed_inset_axes(ax, 0.045, loc=3)
...
plt.savefig('swcan_source_map.png', format='png', bbox_inches='tight', dpi=800)
plt.show()
