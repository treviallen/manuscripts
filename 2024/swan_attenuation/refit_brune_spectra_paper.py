...
def parse_filtering_data():
    ...
    filtdat = []
    # read parameter file
    lines = open('event_filtering_lookup_paper.csv').readlines()[1:]
    for line in lines:
        dat = line.split(',')
...
# do spectral correction    
def spectral_correction(sta, cor_fds, cor_fds_nan, spec_cor):
...
spec_cor = pickle.load(open('sta_spec_correction.pkl', 'rb'))
                    ...
                    if len(rec['channels']) > 0:
                        if rec['ev'] == event: # will need to cahnge to rec['datetime']
                            ...
                            if skip_sta == False:
                                ...
                                # now apply spectral correction
                                cor_fds, cor_fds_nan = spectral_correction(rec['sta'], cor_fds, cor_fds_nan, spec_cor)
