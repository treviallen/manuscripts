...
from numpy import array, arange, log10, exp, logspace, sqrt, isnan, nan, hstack, mean
...
for i, dyfifile in enumerate(dyfifiles):
    ...
    # now plot
    d3 = plt.semilogx(dyfirhyp, dyfimmi, '+', lw=.25, color=cs[ci[i]], ms=5)
    #d3 = plt.semilogx(dyfirepi[0], dyfimmi[0], '+', lw=2., color='0.6', ms=6)
    
    # get stats
    bins = arange(0.05, log10(maxrrup)+0.05, 0.05)
    meanres, stdres, medx, outbins, nperbin = get_binned_stats_mean(bins, log10(dyfirhyp), dyfimmi)
    
    # plot
    d4 = plt.errorbar(10**(medx+bar_off[i]), meanres, yerr=stdres, fmt='s', mec=cs[ci[i]+1], mfc=cs[ci[i]+1], ms=7, elinewidth=1.5, \
...
plt.savefig('min_wp_mmi_atten.png', format='png', dpi=300, bbox_inches='tight')
