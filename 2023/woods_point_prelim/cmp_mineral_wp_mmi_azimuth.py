...
from misc_tools import get_binned_stats_mean, remove_last_cmap_colour
from gmt_tools import cpt2colormap
from numpy import array, arange, log10, exp, logspace, sqrt, isnan, nan, hstack, mean, where
...
mag = 5.9
eqdeps = [6, 12] 
eqlats = [37.910, -37.5063]
eqlons = [-77.936, 146.4022]
ztor = 4.
maxrrup = 410.
...
dyfifiles = ['felt_reports/2011_mineral_dyfi_geo_1km.geojson', 'felt_reports/felt_reports_1km.geojson']
ev_labels = ['2011 Mineral VA', '2021 Woods Point VIC']

ci = [0, 6]
bar_off = [0., 0.01]

for i, dyfifile in enumerate(dyfifiles):
    ...
    for feature in data['features']:
        try:
            ...
            meanlon = mean(array(feature['geometry']['coordinates'][0])[:,0])
            meanlat = mean(array(feature['geometry']['coordinates'][0])[:,1])
    ...
    dyfimmi = array(dyfimmi)
    dyfirepi = []
    dyfiazim = []
    for j in range(0, len(dyfilat)):
        dyfirepi.append(distance(eqlats[i], eqlons[i], dyfilat[j], dyfilon[j])[0])
        dyfiazim.append(distance(eqlats[i], eqlons[i], dyfilat[j], dyfilon[j])[1])
    dyfirhyp = sqrt(array(dyfirepi)**2 + eqdeps[i]**2)
    dyfiazim = array(dyfiazim)
    
    print(min(dyfirhyp))
    
    # now grab data between 100-130 km
    mindist = 100
    maxdist = 150
    idx = where((dyfirhyp >= mindist) & (dyfirhyp <= maxdist))[0]
    
    # now plot
    d3 = plt.plot(dyfiazim[idx], dyfimmi[idx], '+', lw=.25, color=cs[ci[i]], ms=5)
    #d3 = plt.semilogx(dyfirepi[0], dyfimmi[0], '+', lw=2., color='0.6', ms=6)
    
    # get stats
    bins = arange(5, 361, 10)
    meanres, stdres, medx, outbins, nperbin = get_binned_stats_mean(bins, dyfiazim[idx], dyfimmi[idx])
    
    # plot
    d4 = plt.errorbar((medx+bar_off[i]), meanres, yerr=stdres, fmt='s', mec=cs[ci[i]+1], mfc=cs[ci[i]+1], ms=7, elinewidth=1.5, \
                      capsize=5, zorder=1000000, ecolor=cs[ci[i]+1], label=ev_labels[i]) 
...
"""
plt.legend(loc=3, numpoints=1).set_zorder(2000000)
...
plt.savefig('min_wp_mmi_azimuth.png', format='png', dpi=300, bbox_inches='tight')
