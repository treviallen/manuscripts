...
def normalise_data(p, recs, sn_ratio, events):
    ...
    for e in events:
        ...
        if len(mrhyps) > 0:
            ...
            nidx = where((binstrp >= 2.45) & (binstrp < 2.55))[0] # 250 km
...
for p, freq in enumerate(freqs[fidx]):
    ...
    coeffs.append({'nc0': n0, 'r1': r1, 'r2': r2, 'nref':nref, 'freq':freq})
...
trilin_mc0 = reg1.intercept + reg1.slope*log10(freqs[fidx]) # subbing from above

# set to coeffs
for i, c in enumerate(coeffs):
    coeffs[i]['mc0s'] = interp_mc0[i]
    coeffs[i]['mc0f'] = trilin_mc0[i]
