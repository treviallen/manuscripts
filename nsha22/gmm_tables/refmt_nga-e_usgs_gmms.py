...
for xls in xlstables:
    ...
    # first get 3k reference PGA values in g
    ref_pga = []
    for sheet, fwt in zip(sheet_names, modwts):
        if sheet.strip('F') == 'PGA':
            ...
            for row_idx in range(1, xl_sheet.nrows):
                ref_pga.append(float(xl_sheet.cell(row_idx, 2).value))
    
    # now get data for other periods        
    for sheet, fwt in zip(sheet_names, modwts):
        #print(sheet)
        sa = []
        ...
        for row_idx in range(1, xl_sheet.nrows):    # Iterate through rows - skip header
            ...
            ln_pga_ref3k = log(ref_pga[row_idx-1])
            ...
            if vsAdjust == True:
                ...
                ampFact = nga_east_siteamp(targetVS, refT, ln_pga_ref3k, impedance=impedance)[0]
    ...
    # write headers
    if vsAdjust == True:
        outfile = path.join('ngae_usgs_txt_tables', '.'.join((basename, str(targetVS),'mps.txt')))
        
        if weightTab == True:
            outfile_wt = path.join('weighted_usgs_ngae_tables', '.'.join((basename, str(targetVS),'mps.txt')))
            
        if altSigma == False:
            outfile = path.join('ngae_usgs_txt_tables', '.'.join((basename, 'AA13sigma', str(targetVS),'mps.txt')))
