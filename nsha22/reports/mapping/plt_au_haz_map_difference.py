...
from misc_tools import remove_last_cmap_colour, remove_first_cmap_colour
import shapefile
...
system(' '.join(('gmt5 grdmath', grid1, grid2, ' SUB  = map_diff.grd'))) #au_land_mask.grd MUL = map_diff.grd'))) #au_land_mask.grd MUL 

# set small values to zero
#system('gmt5 grdmath map_diff.grd 0.01 GE map_ratio.grd MUL = map_diff.grd')
...
cptfile = '/Users/trev/Documents/DATA/GMT/cpt/cbcPuOr.cpt'
...
cmap = remove_last_cmap_colour(cmap)
cmap = plt.cm.get_cmap('PuOr', ncolours)
bounds = array([-1.0, -0.9, -0.8, -7.0e-02, -6.0e-02, -5.0e-02,
                -4.0e-02, -3.0e-02, -2.0e-02, -1.0e-02,
                -0.0    ,  1.0e-02,  2.0e-02,  3.0e-02,
                 4.0e-02,  5.0e-02,  6.0e-02,  7.0e-02,  0.8, 0.9, 1.0]) 
...
# set title
if pltkey == 'PGA-0.02':
    titlestr = '2% in 50-Year Hazard Difference (NSHA23 - NSHA18) in g'
elif pltkey == 'PGA-0.1':
   titlestr = '10% in 50-Year Hazard Difference (NSHA23 - NSHA18) in g'
...
# now save png file
if pltkey == 'PGA-0.02':
    plt.savefig('maps/nsha23-18_0.02_diff_map.png', dpi=300, format='png', bbox_inches='tight')
elif pltkey == 'PGA-0.1':
    plt.savefig('maps/nsha23-18_0.1_diff_map.png', dpi=300, format='png', bbox_inches='tight')
