from numpy import array, arange, argsort, where, delete, hstack, sqrt, \
                  unique, mean, percentile, log10, ceil, floor, zeros_like, \
...
b_min = -0.3
b_max = 0.3
cmap = plt.get_cmap('BrBG', ncolours)
cmap = plt.get_cmap('PRGn', ncolours)
cmap = plt.get_cmap('RdBu', ncolours)
...
###############################################################################
# load shapes and get diff
###############################################################################

# parse data
src_code18, src_bval18, src_n0, src_class, sf = parse_shp_attributes(shpfile18)
src_code23, src_bval23, src_n023, src_class, sf = parse_shp_attributes(shpfile23)

# get diff
bval_diff = zeros_like(src_bval18)
for i, s18 in enumerate(src_code18):
    for j, s23 in enumerate(src_code23):
        if s23 == s18:
            bval_diff[i] = src_bval23[j] - src_bval18[i]
...
# loop thru b values
for b, n0 in zip(bval_diff, src_n0):
    if not isnan(n0):
...
cax = fig.add_axes([0.2,0.11,0.6,0.035]) # setup colorbar axes.
...
cb.set_label('b-value Difference (NSHA23 - NSHA18)', fontsize=17)
  
# save figure
plt.savefig('maps/diff_2018_2023_b_val.png', fmt='png', dpi=300, bbox_inches='tight')
