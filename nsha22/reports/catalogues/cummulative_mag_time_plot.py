...
from numpy import array, where, hstack, delete, nan, isnan
...
for i, mc in enumerate(mcdat):
    if mc['DATETIME'].year >= 1900:
        if mc['MLREGION'] == 'EA' or mc['MLREGION'] == 'SA':
            evdt.append(mc['DATETIME'])
            
            if not isnan(mc['PREFMW']) or not isnan(mc['MX_ORIGML']):
                if not isnan(mc['PREFMW']):
                       mx_orig.append(mc['PREFMW'])
                elif not isnan(mc['MX_ORIGML']):
                    mx_orig.append(mc['MX_ORIGML'])
            
            elif not isnan(mc['PREFMW']) or not isnan(mc['SeiscompML']):
                if not isnan(mc['PREFMW']):
                    mx_orig.append(mc['PREFMW'])
                elif not isnan(mc['SeiscompML']):
                    mx_orig.append(mc['SeiscompML'])
            
            else:
                mx_orig.append(nan)
            
            mx_rev_ml.append(mc['PREFML_2023'])
            mw_pref.append(mc['PREFMW_2023'])

# add phantom rec for plotting - but won't be plotted

evdt.append(UTCDateTime(2022,12,31,23,59))
mx_orig.append(6.0)
mx_rev_ml.append(6.0)
mw_pref.append(6.0)

evdt = array(evdt)
mx_orig = array(mx_orig)
mx_rev_ml = array(mx_rev_ml)
mw_pref = array(mw_pref)
...
for i, mm in enumerate(minmag):
   ...
   '''
   # #082e41', '#0b5e4b', '#cb6c37', '#00718b', '#b43b3b', '#773775', '#637c6b', '#606f74'
   plt.step(decimal_yrs[mxoidx], range(0, len(mxoidx)), color='#b43b3b', lw=1.5)
   plt.step(decimal_yrs[mxridx], range(0, len(mxridx)), color='#cb6c37', lw=1.5)
   plt.step(decimal_yrs[mwidx], range(0, len(mwidx)), color='#00718b', lw=1.5)
   
   if i == 0:
       plt.legend(('Original Magnitudes', 'Modified $\mathregular{M_L}$', 'Preferred $\mathregular{M_W}$'), loc=2, fontsize=13)
...
plt.savefig('cummulative_mag_time.png', fmt='png', bbox_inches='tight', dpi=300)
plt.show()
