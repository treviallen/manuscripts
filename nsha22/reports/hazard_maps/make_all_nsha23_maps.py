...
periods = ['PGA', 'SA01', 'SA015', 'SA02', 'SA03', 'SA04', 'SA05', 'SA07', 'SA10', 'SA15', 'SA20', 'SA30']
periods = ['SA10']
#periods = ['SA10']
...
#cptfile = 'ch05m151008.cpt' #colour-vision friendly
...
# loop thru periods
for t in periods:
    for p in pltProbability:
        
        mapfolder = 'results_maps_'+t+'_SC_'+sc.replace('.','')
        
        gridfile = path.join(basepath, mapfolder, 'hazard_map-mean_1.csv')
        ...
        for i, key in enumerate([keys[mapidx]]): # just plot 1 for now!
            ...
            if probability == '10%': # or probability == '2%':
                if period == 'PGA':
            ...
            elif probability == '3.3%': # or probability == '2%':
                if period == 'PGA':
                    bounds = array([0, 0.01, 0.02, 0.03, 0.04, 0.06, 0.08, 0.10, 0.14, 0.20, 0.26, 0.32, 0.4])
                elif period == 'SA005' or period == 'SA01' or period == 'SA015' or period == 'SA02'  \
                   or period == 'SA03' or period == 'SA04' or period == 'SA05':
            ...
            # read data
            capfile = path.join('/Users/trev/Documents/Geoscience_Australia/NSHA2023/', 'shared', 'capitals_names.csv')
            ...
            # add GA logo
            ##########################################################################################
            im = plt.imread('/Users/trev/Documents/Geoscience_Australia/NSHA2023/postprocessing/GAlogo.png')
            # set bbox for logo
            imoff = 0.02
            ...
            # load logo
            im = plt.imread('/Users/trev/Documents/Geoscience_Australia/NSHA2023/postprocessing/ccby_narrow.png')
            ...
            # now save png file
            plt.savefig('hazard_map_'+modelName.replace(' ','_')+'.'+period+'.'+probFraction+'.'+siteClass+suffix+'.png', \
            ...
            siteClass = sc
            grdname = path.join('netcdf', modelName.replace(' ','_')+'.'+period+'.'+probFraction+'.SC_'+siteClass+'.grd')
