...
outfile        = split_config_lines(lines[0])[0]
uhsfile        = split_config_lines(lines[1])[0]
outputdir      = split_config_lines(lines[2])[0]
sitelistfile   = split_config_lines(lines[3])[0]
places         = split_config_lines(lines[4])
...
for j, pi in enumerate(probidx):
    ...
    for i, place in enumerate(places):
        for uhs in uhsDict:
            if place == uhs['place']:
                if pltLog == True:
                    if plt1170 == True:
                        normSA = uhs[probabilities[pi]] / (2 * uhs[probabilities[pi]][0] / 3)
                    ...
                    if plt1170 == True:
                        #normSA = uhs[probabilities[pi]] / (2 * uhs[probabilities[pi]][0] / 3) # for notional UHS
    ...
    if plt1170 == True:
        plt.ylabel('Normalised '+str('%0.0f' % (100*float(probabilities[pi])))+'% in 50-year AEP Sa', fontsize=15)
    else:
    ...
    if j == 1:
        ...
        axins = inset_axes(ax,
                       width="29%",  # width = 30% of parent_bbox
...
if plt1170 == True:
    ...
    pngfile  = path.join('uhs_plots', outfile)
