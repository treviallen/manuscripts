...
from numpy import arange, mean, percentile, array, unique, where, argsort, vstack, diff
...
for i, ed in enumerate(evdt):
    ...
    plt.plot(x, y, 'o', markerfacecolor=cs[colidx], markeredgecolor='k', markeredgewidth=0.5, \
             markersize=6 + evml[i]*2.2, zorder=zo)
...
ticks = arange(0, 1.1, 1./7.)
months = range(6, 13)
labels = []
for m in months:
    tick = datetime(2012, m, 1)
    labels.append(tick.strftime('%d-%b-%Y')[1:])
tick = datetime(2013, 1, 1)
labels.append(tick.strftime('%d-%b-%Y')[1:])

cmap2 = cmap.from_list('custom', cs, N=7) #, ncols
#norm = mpl.colors.BoundaryNorm(ticks, cmap.N)
 
cax = fig.add_axes([0.76,0.3,0.02,0.4]) # setup colorbar axes.
cb = colorbar.ColorbarBase(cax, cmap=cmap2, orientation='vertical') # norm=norm
cb.set_ticks(ticks)
cb.set_ticklabels(labels)

xtxt = ax.get_xlim()[1] * 0.02            
ytxt = ax.get_ylim()[1] * 0.02
#plt.text(xtxt, ytxt, 'A', size=20, ha='left', va='bottom', weight='normal')


plt.savefig('moe_map_epicentres_time.png', format='png', bbox_inches='tight', dpi=300)
plt.show()
